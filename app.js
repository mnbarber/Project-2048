...
function addNum() {
  ...
  if (usableTiles === 0) {
    return; // no available tiles to add a 2 to
  }
...
function combineRow() {
  for (let i = 0; i < 15; i++) {
    if (tiles[i].innerHTML === tiles[i + 1].innerHTML) {
      let combinedTile =
        parseInt(tiles[i].innerHTML) + parseInt(tiles[i + 1].innerHTML);
...
function combineColumn() {
  for (let i = 0; i < 12; i++) {
    if (tiles[i].innerHTML === tiles[i + width].innerHTML) {
      let combinedTile =
        parseInt(tiles[i].innerHTML) + parseInt(tiles[i + width].innerHTML);
      tiles[i].innerHTML = combinedTile;
...
function checkForLose() {
  let usableTiles = 0;
  for (let i = 0; i < 16; i++) {
    if (tiles[i].innerHTML == "" || checkNeighbors(i)) {
        console.log('empty');
        usableTiles++;
    }
  }
...
function checkNeighbors(i) {
  const tile = tiles[i].innerHTML || '';
  const up = tiles[i - 1]?.innerHTML;
  const left = tiles[i - width]?.innerHTML;
  const right = tiles[i + width]?.innerHTML;
  const down = tiles[i + 1]?.innerHTML;
  return [up, down, left, right].includes(tile);
